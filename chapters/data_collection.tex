...
Help: Uncertainty Alignment for Large Language Model Planners} \cite{ren2023robotsaskhelpuncertainty}
can be seen in Listing \ref{lst:knowno_prompt}. This prompt has been taken
directly from the example available in their interactive demo\footnote{\url{https://tmp.com}},
which is accessible online. The prompt follows a structured approach that allows
the language model to process information about the environment, understand the
task, and select the best action from a predefined set of choices.

Since we applied the same uncertainty analysis to the model's responses, we
opted to maintain a similar structure for our prompts.
...
Our heatmaps are a visual representation of the probability distribution of
actions taken within each cell of the map. They provide insights into the model's
decision-making process by displaying the likelihood of selecting specific actions
...
which visually encodes the probability distribution for each action. An example of
such a heatmap is shown in Figure \ref{fig:heatmap_example}. Only the
probability of the retained actions is displayed, with the color intensity indicating
...
A total of 36 test cases have been conducted, and the resulting heatmaps are stored
...
This structure ensures that, for each map size, both pickup and delivery goals
are tested, and in some cases also other specific goals that will help us analyze
the results. The location of the parcel and the delivery tile are varied across
different test cases. The rationale behind the selection of goal positions will be
discussed in Chapter \ref{cha:results_discussion}.

\subsubsection{Example}

To better understand how to interpret these heatmaps, let us analyze a specific case.
In Figure \ref{fig:heatmap_example}, we observe a heatmap generated for a 5×5 grid
where the goal is to deliver a parcel at the center of the map.

Consider the cell at coordinates $(2,1)$, which is located directly to the left
of the goal. The KnowNo framework produced the following output for this cell:
\begin{verbatim}
  ...
  {
    "x": 2,
...
From this data, we observe that the model assigned similar probabilities to
three actions: moving Right (34.14\%), Up (31.85\%), and Down (21.73\%). The Right
action was ultimately chosen as the most likely. Meanwhile, actions such as Left
(4.10\%), Take (T) (4.24\%), and Ship (S) (3.93\%) were discarded by the KnowNo
framework. The remaining probabilities were then normalized to sum to 100\%.
...
Once collected the data for a heatmap, we computed for each cell the correct
actions. Then we summed for each cell the probability given to the correct actions.
As we can see the heatmap in Figure \ref{fig:correctness_heatmap} that is
related to the heatmap in Figure \ref{fig:heatmap_example}.

For example, let's take the top right cell and let's analyze it.